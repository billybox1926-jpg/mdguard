...
def check(file: Path, line: str, lineno: int, ctx: dict, config: dict) -> list[LintIssue]:
...
def post_check(file: Path, ctx: dict, config: dict) -> list[LintIssue]:
    ...
    for lineno, anchor in ctx.get('anchor_refs', []):
        if anchor and anchor not in anchors:
            issues.append(LintIssue(file, lineno, NAME, f"internal heading anchor not found: #{anchor}"))
    return issues
